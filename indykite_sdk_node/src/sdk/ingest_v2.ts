 ...
 * const sdk = await IngestClientV2.createInstance();
 */
export class IngestClientV2 {
  private client: IngestAPIClient;
  constructor(sdk: SdkClient) {
    this.client = sdk.client as IngestAPIClient;
  }
  static createInstance(appCredential?: string | unknown): Promise<IngestClientV2> {
    return new Promise<IngestClientV2>((resolve, reject) => {
      SdkClient.createIdentityInstance(IngestAPIClient, appCredential)
        .then((sdk) => {
          resolve(new IngestClientV2(sdk));
        })
        .catch((err) => {
          reject(err);
        });
